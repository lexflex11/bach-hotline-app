...
export default function MoodTab({ setTab }) {
  ...
  if (!selected && !customMode) return (
    <div>
      ...
      <div style={{fontSize:13,color:HOT,fontFamily:"'Plus Jakarta Sans',sans-serif",marginBottom:14,opacity:0.8}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        {AESTHETICS.map(a => (
          <button key={a.id} onClick={() => setSelected(a.id)}
            ...
            <div style={{fontSize:13,fontWeight:400,fontFamily:"'Plus Jakarta Sans',sans-serif",color:DARK,marginBottom:4}}>{a.name}</div>
            <div style={{fontSize:11,color:HOT,fontFamily:"'Plus Jakarta Sans',sans-serif",lineHeight:1.4,opacity:0.8}}>{a.vibe}</div>
        ...
        <button onClick={() => setCustomMode(true)}
          ...
          <div style={{fontSize:13,fontWeight:400,fontFamily:"'Plus Jakarta Sans',sans-serif",color:HOT,marginBottom:4}}>Build Your Own</div>
          <div style={{fontSize:11,color:HOT,fontFamily:"'Plus Jakarta Sans',sans-serif",lineHeight:1.4,opacity:0.8}}>Name your vibe, pick your colors, get a custom AI décor plan</div>
        </button>
      </div>
    </div>
  );
  ...
  if (customMode && !selected) return (
    <div>
      ...
      <div style={{...C, marginBottom:12}}>
        <div style={{fontSize:13,fontWeight:400,fontFamily:"'Plus Jakarta Sans',sans-serif",color:DARK,marginBottom:6}}>What's your theme called?</div>
        <input
          value={customName}
          onChange={e => setCustomName(e.target.value)}
          placeholder="e.g. Pink Safari, Vegas Glam, Coastal Queen..."
          style={{width:"100%",padding:"12px",borderRadius:10,border:`1.5px solid ${customName?HOT:BORDER}`,fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:13,color:DARK,background:WHITE,boxSizing:"border-box"}}
        />
      </div>

      <div style={{...C, marginBottom:12}}>
        <div style={{fontSize:13,fontWeight:400,fontFamily:"'Plus Jakarta Sans',sans-serif",color:DARK,marginBottom:6}}>Describe the vibe</div>
        <textarea
          value={customVibe}
          onChange={e => setCustomVibe(e.target.value)}
          placeholder="e.g. Luxury resort meets wild west. Think gold, turquoise, and cowboy hats by the pool..."
          rows={3}
          style={{width:"100%",padding:"12px",borderRadius:10,border:`1.5px solid ${customVibe?HOT:BORDER}`,fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:13,color:DARK,background:WHITE,boxSizing:"border-box",resize:"none"}}
        />
      </div>

      <div style={{...C, marginBottom:16}}>
        <div style={{fontSize:13,fontWeight:400,fontFamily:"'Plus Jakarta Sans',sans-serif",color:DARK,marginBottom:6}}>Pick your main color</div>
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          ...
          />
          <div>
            <div style={{fontSize:13,fontFamily:"'Plus Jakarta Sans',sans-serif",color:DARK,fontWeight:700}}>{customColors.toUpperCase()}</div>
            <div style={{fontSize:11,color:"#bbb",fontFamily:"'Plus Jakarta Sans',sans-serif",marginTop:2}}>AI will build your full palette from this</div>
          </div>
        </div>
      </div>
      ...
      {(!customName || !customVibe) && (
        <div style={{textAlign:"center",fontSize:11,color:"#bbb",fontFamily:"'Plus Jakarta Sans',sans-serif",marginTop:8}}>
  ...
  return (
    <div>
      ...
      <div style={{borderRadius:18,padding:"20px 16px",background:SOFT,border:`1.5px solid ${MID}`,marginBottom:14,textAlign:"center"}}>
        ...
        <div style={{fontSize:20,fontWeight:400,fontFamily:"'Plus Jakarta Sans',sans-serif",color:DARK}}>{aesthetic.name}</div>
        <div style={{fontSize:13,color:HOT,fontFamily:"'Plus Jakarta Sans',sans-serif",marginTop:6,lineHeight:1.5,fontStyle:"italic"}}>"{aesthetic.vibe}"</div>
        <div style={{display:"flex",justifyContent:"center",gap:8,marginTop:14,flexWrap:"wrap"}}>
          {aesthetic.colors.map((c,i) => (
            <div key={i} style={{textAlign:"center"}}>
              ...
              <div style={{fontSize:9,color:DARK,fontFamily:"'Plus Jakarta Sans',sans-serif",opacity:0.7}}>{aesthetic.palette[i]}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {aesthetic.outfits?.length > 0 && <div style={{...C,marginBottom:12}}>
        <div style={{fontSize:13,fontWeight:400,fontFamily:"'Plus Jakarta Sans',sans-serif",color:DARK,marginBottom:10}}>What to Wear</div>
        {aesthetic.outfits.map((item,i) => (
          <div key={i} style={{display:"flex",gap:10,marginBottom:8,alignItems:"center"}}>
            ...
            <div style={{fontSize:13,fontFamily:"'Plus Jakarta Sans',sans-serif",color:DARK}}>{item}</div>
          </div>
        ))}
      </div>}
      ...
      {aesthetic.decor?.length > 0 && <div style={{...C,marginBottom:12}}>
        <div style={{fontSize:13,fontWeight:400,fontFamily:"'Plus Jakarta Sans',sans-serif",color:DARK,marginBottom:10}}>Décor Ideas</div>
      ...
      {aesthetic.shots?.length > 0 && <div style={{...C,marginBottom:12}}>
        <div style={{fontSize:13,fontWeight:400,fontFamily:"'Plus Jakarta Sans',sans-serif",color:DARK,marginBottom:10}}>Photo Moments</div>
      ...
      {aesthetic.hashtags?.length > 0 && <div style={{...C,marginBottom:12}}>
        <div style={{fontSize:13,fontWeight:400,fontFamily:"'Plus Jakarta Sans',sans-serif",color:DARK,marginBottom:10}}>Hashtag Pack</div>
        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
          {aesthetic.hashtags.map((tag,i) => (
            <span key={i} style={{fontSize:11,padding:"4px 11px",borderRadius:50,background:SOFT,border:`1.5px solid ${MID}`,color:HOT,fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:600}}>{tag}</span>
          ))}
        </div>
      </div>}
      ...
      <div style={{...C, marginBottom:12, border:`2px solid ${HOT}`, background:SOFT}}>
        ...
        {!photo ? (
          ...
          <div>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:13,marginBottom:12}}>
        ...
        {loading && (
          <div style={{textAlign:"center",padding:"24px 0",color:HOT,fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:13}}>
        ...
        {result && (
          <div style={{marginTop:14}}>
            <div style={{fontSize:17,fontWeight:400,fontFamily:"'Plus Jakarta Sans',sans-serif",color:DARK,marginBottom:8}}>{result.headline}</div>
            <p style={{fontSize:13,lineHeight:1.6,color:DARK,fontFamily:"'Plus Jakarta Sans',sans-serif",marginBottom:12,opacity:0.85}}>{result.vision}</p>

            {result.keyPieces?.length > 0 && (
              <div style={{marginBottom:12}}>
                <div style={{fontSize:11,fontWeight:700,color:HOT,fontFamily:"'Plus Jakarta Sans',sans-serif",textTransform:"uppercase",letterSpacing:1,marginBottom:8}}>Key Décor Pieces</div>
            ...
            {result.lighting && (
              <div style={{padding:"10px 12px",borderRadius:10,background:WHITE,border:`1px solid ${BORDER}`,marginBottom:10}}>
                <span style={{fontSize:11,fontWeight:700,color:HOT,fontFamily:"'Plus Jakarta Sans',sans-serif"}}> Lighting: </span>
                <span style={{fontSize:12,color:DARK,fontFamily:"'Plus Jakarta Sans',sans-serif"}}>{result.lighting}</span>
              </div>
            )}

            {result.photoMoment && (
              <div style={{padding:"10px 12px",borderRadius:10,background:WHITE,border:`1px solid ${BORDER}`,marginBottom:12}}>
                <span style={{fontSize:11,fontWeight:700,color:HOT,fontFamily:"'Plus Jakarta Sans',sans-serif"}}> Photo Moment: </span>
                <span style={{fontSize:12,color:DARK,fontFamily:"'Plus Jakarta Sans',sans-serif"}}>{result.photoMoment}</span>
              </div>
            )}
            ...
            {result.shoppingList?.length > 0 && (
              <div style={{background:WHITE,border:`1.5px solid ${BORDER}`,borderRadius:12,padding:"11px 13px",marginBottom:12}}>
                <div style={{fontSize:10,color:HOT,fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:700,textTransform:"uppercase",letterSpacing:1,marginBottom:8}}> Shopping List</div>
                {result.shoppingList.map((item,i) => (
                  <div key={i} style={{fontSize:12,fontFamily:"'Plus Jakarta Sans',sans-serif",color:DARK,opacity:0.8,marginBottom:4}}>• {item}</div>
                ))}
              </div>
            )}
      ...
      <div style={{...C,background:SOFT,border:`1.5px solid ${MID}`}}>
        <div style={{fontSize:13,fontWeight:400,fontFamily:"'Plus Jakarta Sans',sans-serif",color:DARK,marginBottom:12}}>Shop This Look</div>
              ...
              <a key={id} href={p.url} target="_blank" rel="noreferrer" style={{flex:1,textDecoration:"none"}}>
                <div style={{background:WHITE,border:`1.5px solid ${BORDER}`,borderRadius:12,overflow:"hidden",textAlign:"center"}}>
                  ...
                  <div style={{padding:"6px 8px"}}>
                    <div style={{fontSize:10,fontFamily:"'Plus Jakarta Sans',sans-serif",color:DARK,fontWeight:600,lineHeight:1.3}}>{p.name}</div>
                    <div style={{fontSize:11,color:PUNCH,fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:700,marginTop:2}}>${p.price.toFixed(2)}</div>
                  </div>
                </div>
              </a>
            );
