...
const inp  = { width:"100%", border:`1.5px solid ${BORDER}`, borderRadius:12, padding:"11px 14px", fontFamily:"'Plus Jakarta Sans',sans-serif", fontSize:14, background:PAGE, color:DARK, outline:"none", boxSizing:"border-box" };
const lbl  = { fontSize:12, fontWeight:700, fontFamily:"'Plus Jakarta Sans',sans-serif", color:HOT, textTransform:"uppercase", letterSpacing:"0.8px", marginBottom:6, display:"block" };
function SH({ title, sub }) {
  return <div style={{ marginBottom:18 }}><div style={{ fontSize:20, fontWeight:800, fontFamily:"'Plus Jakarta Sans',sans-serif", color:DARK }}>{title}</div>{sub&&<div style={{ fontSize:12, color:"#aaa", fontFamily:"'Plus Jakarta Sans',sans-serif", marginTop:3 }}>{sub}</div>}</div>;
}
  ...
  return (
    <div>
      ...
      <div style={{ ...card, textAlign:"center", padding:"28px 20px" }}>
        ...
        <div style={{ fontSize:16, fontWeight:800, fontFamily:"'Plus Jakarta Sans',sans-serif", color:DARK, marginBottom:8 }}>One quick setup needed</div>
        <div style={{ fontSize:13, color:"#888", fontFamily:"'Plus Jakarta Sans',sans-serif", lineHeight:1.6, marginBottom:20 }}>Polls use Firebase (free) to sync votes across your group in real time. Takes about 5 minutes.</div>
        <div style={{ background:SOFT, borderRadius:14, padding:"16px", marginBottom:16, textAlign:"left" }}>
          {["Go to console.firebase.google.com",'Click "Add project" → name it Bach Hotline','Click the </> icon → register a web app',"Copy the firebaseConfig values into src/firebase.js","Enable Firestore Database → Start in test mode","In Terminal: npm install  (in the src folder)"].map((step,i)=>(
            <div key={i} style={{ display:"flex", gap:10, marginBottom:8, alignItems:"flex-start" }}>
              ...
              <div style={{ fontSize:12, color:DARK, fontFamily:"'Plus Jakarta Sans',sans-serif", lineHeight:1.5, paddingTop:2 }}>{step}</div>
            </div>
          ))}
        </div>
        <div style={{ fontSize:11, color:"#aaa", fontFamily:"'Plus Jakarta Sans',sans-serif" }}>Free · No credit card · ~5 minutes</div>
      </div>
    </div>
  );
}
...
function PollList({ user, onSelect, onCreateNew }) {
  ...
  return (
    <div>
      ...
      {loading ? <div style={{ textAlign:"center", padding:40, color:"#aaa", fontFamily:"'Plus Jakarta Sans',sans-serif" }}>Loading...</div>
        : polls.length === 0 ? <div style={{ ...card, textAlign:"center", padding:"32px 20px" }}><div style={{ fontSize:36, marginBottom:8 }}></div><div style={{ color:"#aaa", fontFamily:"'Plus Jakarta Sans',sans-serif", fontSize:13 }}>No polls yet! Create one and share it with your group.</div></div>
        : polls.map(poll => {
          ...
          return (
            <button key={poll.id} onClick={()=>onSelect(poll.id)} style={{ ...card, display:"block", width:"100%", textAlign:"left", cursor:"pointer", padding:"14px 16px" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                <div style={{ flex:1 }}>
                  {poll.tripName&&<div style={{ fontSize:10, fontWeight:700, color:HOT, fontFamily:"'Plus Jakarta Sans',sans-serif", textTransform:"uppercase", letterSpacing:"0.8px", marginBottom:3 }}>{poll.tripName}</div>}
                  <div style={{ fontSize:15, fontWeight:400, fontFamily:"'Plus Jakarta Sans',sans-serif", color:DARK, marginBottom:4 }}>{poll.question}</div>
                  <div style={{ fontSize:11, color:"#aaa", fontFamily:"'Plus Jakarta Sans',sans-serif" }}>{total} vote{total!==1?"s":""}</div>
                </div>
                <div style={{ marginLeft:10 }}>
                  {hasVoted ? <span style={{ fontSize:11, background:SOFT, color:HOT, padding:"3px 10px", borderRadius:50, fontWeight:700, fontFamily:"'Plus Jakarta Sans',sans-serif" }}>Voted </span>
...
function CreatePoll({ user, onCreated, onBack }) {
  ...
  if (step === 0) return (
    <div>
      <button onClick={onBack} style={{ background:"none", border:"none", color:HOT, fontFamily:"'Plus Jakarta Sans',sans-serif", fontWeight:700, fontSize:13, cursor:"pointer", marginBottom:16, padding:0 }}>← Back</button>
      ...
      {TEMPLATES.map((t,i)=>(
        <button key={i} onClick={()=>{ setQuestion(t.label.replace(/^[^\s]+\s+/,"")); setOptions([...t.options.filter(Boolean),"",""].slice(0,4)); setStep(1); }}
          ...
          <div style={{ fontSize:14, fontWeight:700, fontFamily:"'Plus Jakarta Sans',sans-serif", color:DARK }}>{t.label}</div>
  ...
  if (step === 1) return (
    <div>
      <button onClick={()=>setStep(0)} style={{ background:"none", border:"none", color:HOT, fontFamily:"'Plus Jakarta Sans',sans-serif", fontWeight:700, fontSize:13, cursor:"pointer", marginBottom:16, padding:0 }}>← Back</button>
  ...
  return (
    <div>
      ...
      <div style={{ ...card, textAlign:"center" }}>
        ...
        <div style={{ fontSize:13, fontFamily:"'Plus Jakarta Sans',sans-serif", color:"#888", marginBottom:12 }}>Anyone with this link can vote!</div>
...
function VoteScreen({ pollId, onBack }) {
  ...
  if(loading) return <div style={{ textAlign:"center", padding:40, color:"#aaa", fontFamily:"'Plus Jakarta Sans',sans-serif" }}>Loading poll...</div>;
  if(!poll)   return <div style={{ textAlign:"center", padding:40, color:"#aaa", fontFamily:"'Plus Jakarta Sans',sans-serif" }}>Poll not found.</div>;
  ...
  return (
    <div>
      <button onClick={onBack} style={{ background:"none", border:"none", color:HOT, fontFamily:"'Plus Jakarta Sans',sans-serif", fontWeight:700, fontSize:13, cursor:"pointer", marginBottom:16, padding:0 }}>← All polls</button>
      <div style={card}>
        {poll.tripName&&<div style={{ fontSize:11, fontWeight:700, fontFamily:"'Plus Jakarta Sans',sans-serif", color:HOT, textTransform:"uppercase", letterSpacing:"0.8px", marginBottom:6 }}>{poll.tripName}</div>}
        <div style={{ fontSize:19, fontWeight:800, fontFamily:"'Plus Jakarta Sans',sans-serif", color:DARK, marginBottom:4 }}>{poll.question}</div>
        <div style={{ fontSize:11, color:"#aaa", fontFamily:"'Plus Jakarta Sans',sans-serif", marginBottom:20 }}>{total} vote{total!==1?"s":""} · by {poll.createdBy}</div>
              ...
              <button onClick={async()=>{ if(voted)return; setVoted(true); setVotedOptId(opt.id); const db=getDb(); await updateDoc(doc(db,"polls",pollId),{[`votes.${opt.id}`]:arrayUnion(voterId)}); }} disabled={voted}
                ...
                <div style={{ position:"relative", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                    {isMe&&<span></span>}
                    <span style={{ fontSize:14, fontFamily:"'Plus Jakarta Sans',sans-serif", fontWeight:isMe?700:500, color:DARK }}>{opt.text}</span>
                    {isTop&&<span style={{ fontSize:11, background:`linear-gradient(135deg,#f472b0,${HOT})`, color:WHITE, padding:"2px 8px", borderRadius:50, fontWeight:700, fontFamily:"'Plus Jakarta Sans',sans-serif" }}>Leading</span>}
                  </div>
                  {voted&&<span style={{ fontSize:13, fontWeight:700, fontFamily:"'Plus Jakarta Sans',sans-serif", color:isMe?HOT:"#aaa" }}>{pct}%</span>}
                </div>
              </button>
            </div>
          );
        })}
        {!voted&&<div style={{ textAlign:"center", fontSize:12, color:"#bbb", fontFamily:"'Plus Jakarta Sans',sans-serif", marginTop:8 }}>Tap an option to vote</div>}
        {voted&&<div style={{ textAlign:"center", fontSize:12, color:HOT, fontFamily:"'Plus Jakarta Sans',sans-serif", fontWeight:700, marginTop:8 }}>You voted! Results update live </div>}
      </div>
...
function AccountGate({ onSignUp }) {
  return (
    <div style={{textAlign:"center",padding:"48px 24px"}}>
      ...
      <button onClick={onSignUp} style={{background:`linear-gradient(135deg,#f472b0,${HOT})`,color:WHITE,border:"none",borderRadius:50,padding:"13px 32px",fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:14,fontWeight:700,cursor:"pointer",width:"100%",maxWidth:280}}>
        Create Free Account
      </button>
