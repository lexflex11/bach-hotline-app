...
function SH({ title, sub }) {
  return <div style={{ marginBottom:18 }}><div style={{ fontSize:20, fontWeight:800, fontFamily:"'Playfair Display',Georgia,serif", color:DARK }}>{title}</div>{sub&&<div style={{ fontSize:12, color:"#aaa", fontFamily:"'DM Sans',sans-serif", marginTop:3 }}>{sub}</div>}</div>;
}
  ...
  return (
    <div>
      ...
      <div style={{ ...card, textAlign:"center", padding:"28px 20px" }}>
        ...
        <div style={{ fontSize:13, color:"#888", fontFamily:"'DM Sans',sans-serif", lineHeight:1.6, marginBottom:20 }}>Polls use Firebase (free) to sync votes across your group in real time. Takes about 5 minutes.</div>
        ...
        <div style={{ fontSize:11, color:"#aaa", fontFamily:"'DM Sans',sans-serif" }}>Free · No credit card · ~5 minutes</div>
      </div>
    </div>
  );
}
...
function PollList({ user, onSelect, onCreateNew }) {
  ...
  return (
    <div>
      ...
      {loading ? <div style={{ textAlign:"center", padding:40, color:"#aaa", fontFamily:"'DM Sans',sans-serif" }}>Loading...</div>
        : polls.length === 0 ? <div style={{ ...card, textAlign:"center", padding:"32px 20px" }}><div style={{ fontSize:36, marginBottom:8 }}>🗳️</div><div style={{ color:"#aaa", fontFamily:"'DM Sans',sans-serif", fontSize:13 }}>No polls yet! Create one and share it with your group.</div></div>
        : polls.map(poll => {
          ...
          return (
            <button key={poll.id} onClick={()=>onSelect(poll.id)} style={{ ...card, display:"block", width:"100%", textAlign:"left", cursor:"pointer", padding:"14px 16px" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                <div style={{ flex:1 }}>
                  {poll.tripName&&<div style={{ fontSize:10, fontWeight:700, color:HOT, fontFamily:"'DM Sans',sans-serif", textTransform:"uppercase", letterSpacing:"0.8px", marginBottom:3 }}>{poll.tripName}</div>}
                  ...
                  <div style={{ fontSize:11, color:"#aaa", fontFamily:"'DM Sans',sans-serif" }}>{total} vote{total!==1?"s":""}</div>
                </div>
...
function CreatePoll({ user, onCreated, onBack }) {
  ...
  if (step === 0) return (
    <div>
      <button onClick={onBack} style={{ background:"none", border:"none", color:HOT, fontFamily:"'DM Sans',sans-serif", fontWeight:700, fontSize:13, cursor:"pointer", marginBottom:16, padding:0 }}>← Back</button>
      ...
      {TEMPLATES.map((t,i)=>(
        <button key={i} onClick={()=>{ setQuestion(t.label.replace(/^[^\s]+\s+/,"")); setOptions([...t.options.filter(Boolean),"",""].slice(0,4)); setStep(1); }}
          ...
          <div style={{ fontSize:14, fontWeight:700, fontFamily:"'DM Sans',sans-serif", color:DARK }}>{t.label}</div>
  ...
  if (step === 1) return (
    <div>
      <button onClick={()=>setStep(0)} style={{ background:"none", border:"none", color:HOT, fontFamily:"'DM Sans',sans-serif", fontWeight:700, fontSize:13, cursor:"pointer", marginBottom:16, padding:0 }}>← Back</button>
  ...
  return (
    <div>
      ...
      <div style={{ ...card, textAlign:"center" }}>
        ...
        <div style={{ fontSize:13, fontFamily:"'DM Sans',sans-serif", color:"#888", marginBottom:12 }}>Anyone with this link can vote!</div>
...
function VoteScreen({ pollId, onBack }) {
  ...
  if(loading) return <div style={{ textAlign:"center", padding:40, color:"#aaa", fontFamily:"'DM Sans',sans-serif" }}>Loading poll...</div>;
  if(!poll)   return <div style={{ textAlign:"center", padding:40, color:"#aaa", fontFamily:"'DM Sans',sans-serif" }}>Poll not found.</div>;
  ...
  return (
    <div>
      <button onClick={onBack} style={{ background:"none", border:"none", color:HOT, fontFamily:"'DM Sans',sans-serif", fontWeight:700, fontSize:13, cursor:"pointer", marginBottom:16, padding:0 }}>← All polls</button>
      <div style={card}>
        {poll.tripName&&<div style={{ fontSize:11, fontWeight:700, fontFamily:"'DM Sans',sans-serif", color:HOT, textTransform:"uppercase", letterSpacing:"0.8px", marginBottom:6 }}>{poll.tripName}</div>}
        ...
        <div style={{ fontSize:11, color:"#aaa", fontFamily:"'DM Sans',sans-serif", marginBottom:20 }}>{total} vote{total!==1?"s":""} · by {poll.createdBy}</div>
        ...
        {!voted&&<div style={{ textAlign:"center", fontSize:12, color:"#bbb", fontFamily:"'DM Sans',sans-serif", marginTop:8 }}>Tap an option to vote</div>}
        {voted&&<div style={{ textAlign:"center", fontSize:12, color:HOT, fontFamily:"'DM Sans',sans-serif", fontWeight:700, marginTop:8 }}>You voted! Results update live 🗳️</div>}
      </div>
