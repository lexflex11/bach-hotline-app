...
function AccountGate({ onSignUp }) {
  return (
    <div style={{textAlign:"center", padding:"48px 24px"}}>
      ...
      <h2 style={{fontFamily:"'Plus Jakarta Sans',sans-serif", fontSize:22, fontWeight:700, color:HOT, margin:"0 0 10px"}}>
        Members-Only Deals
      </h2>
      <p style={{fontSize:13, color:"#888", fontFamily:"'Plus Jakarta Sans',sans-serif", lineHeight:1.7, marginBottom:10}}>
      ...
      <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:28, textAlign:"left"}}>
        {[
          ...
          <div key={d.icon} style={{background:SOFT, border:`1px solid ${MID}`, borderRadius:14, padding:"12px 12px"}}>
            ...
            <div style={{fontSize:12, fontWeight:700, color:DARK, fontFamily:"'Plus Jakarta Sans',sans-serif"}}>{d.label}</div>
            <div style={{fontSize:10, color:"#aaa", fontFamily:"'Plus Jakarta Sans',sans-serif", marginTop:3, lineHeight:1.4}}>{d.desc}</div>
          </div>
        ))}
      </div>
    ...
    <div>
      ...
      {!notif && (
        <div style={{...C, marginBottom:14, background:SOFT, border:`1.5px solid ${MID}`}}>
          <div style={{fontSize:11, color:HOT, fontFamily:"'Plus Jakarta Sans',sans-serif", fontWeight:700, textTransform:"uppercase", letterSpacing:1, marginBottom:6}}> Enable Push Notifications</div>
          <div style={{fontSize:12, color:DARK, fontFamily:"'Plus Jakarta Sans',sans-serif", opacity:0.75, marginBottom:12}}>Turn on alerts so we ping you the moment a deal drops!</div>
          <button onClick={()=>setNotif(true)} style={{...BP, width:"100%", padding:"11px", fontSize:13}}>Enable Deal Alerts</button>
        </div>
      )}
      {notif && (
        <div style={{padding:"10px 14px", borderRadius:12, background:"rgba(46,125,50,0.08)", border:"1px solid rgba(46,125,50,0.25)", marginBottom:14, fontFamily:"'Plus Jakarta Sans',sans-serif", fontSize:13, color:GREEN}}>
      ...
      {triggered.length > 0 && (
        <div style={{marginBottom:16}}>
          <div style={{fontSize:11, fontWeight:700, color:PUNCH, fontFamily:"'Plus Jakarta Sans',sans-serif", textTransform:"uppercase", letterSpacing:1, marginBottom:10}}> DEALS FOUND — Act Fast!</div>
          {triggered.map(a => (
            <div key={a.id} style={{...C, marginBottom:10, background:"rgba(46,125,50,0.05)", border:"2px solid rgba(46,125,50,0.3)", position:"relative"}}>
              ...
              <div style={{display:"flex", alignItems:"center", gap:12}}>
                ...
                <div style={{flex:1}}>
                  <div style={{fontSize:13, fontWeight:400, fontFamily:"'Plus Jakarta Sans',sans-serif", color:DARK}}>{a.label}</div>
                  <div style={{display:"flex", gap:12, marginTop:5, fontFamily:"'Plus Jakarta Sans',sans-serif", fontSize:11}}>
      ...
      {watching.length > 0 && (
        <div style={{marginBottom:16}}>
          <div style={{fontSize:11, fontWeight:700, color:HOT, fontFamily:"'Plus Jakarta Sans',sans-serif", textTransform:"uppercase", letterSpacing:1, marginBottom:10}}> Watching</div>
          {watching.map(a => (
            <div key={a.id} style={{...C, marginBottom:8}}>
              <div style={{display:"flex", alignItems:"center", gap:12}}>
                ...
                <div style={{flex:1}}>
                  <div style={{fontSize:13, fontWeight:400, fontFamily:"'Plus Jakarta Sans',sans-serif", color:DARK}}>{a.label}</div>
                  <div style={{fontSize:11, color:HOT, fontFamily:"'Plus Jakarta Sans',sans-serif", marginTop:3, opacity:0.75}}>
      ...
      {showNew ? (
        <div style={{...C, border:`1.5px solid ${HOT}`}}>
          <div style={{fontSize:14, fontWeight:400, fontFamily:"'Plus Jakarta Sans',sans-serif", color:DARK, marginBottom:12}}> New Alert</div>
          ...
          <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:6, marginBottom:12}}>
            {ALERT_TYPES.map(t => (
              <button key={t.id} onClick={()=>setNewA(p=>({...p, type:t.id}))} style={{
                ...
                color:DARK, fontFamily:"'Plus Jakarta Sans',sans-serif", fontSize:11, fontWeight:600,
              }}>
                {t.icon} {t.label}
              </button>
            ))}
