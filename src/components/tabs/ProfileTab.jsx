    ...
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:16 }}>
        {[["✈️","Flights Saved","3"],["🏠","Stays Saved","5"],["🛒","Cart Items",String(cart.length)],["🔔","Active Alerts","4"]].map(([icon,label,val])=>(
          <div key={label} style={{ ...C, textAlign:"center", padding:"14px 10px" }}>
            ...
            <div style={{ fontSize:10, color:"#bbb", fontFamily:"'DM Sans',sans-serif" }}>{label}</div>
          </div>
        ))}
      </div>
      <SH title="My Trips 🗺️" />
      {[{ dest:"Nashville",emoji:"🎸",dates:"Jun 14–17",members:8,status:"Planning" },{ dest:"Miami",emoji:"🌴",dates:"Aug 2–5",members:10,status:"Saved" }].map((t,i)=>(
        <div key={i} style={{ ...C, marginBottom:10, display:"flex", alignItems:"center", gap:14 }}>
          ...
          <div style={{ flex:1 }}><div style={{ fontSize:14, fontWeight:700, fontFamily:"'Playfair Display',Georgia,serif", color:DARK }}>{t.dest} Bachelorette</div><div style={{ fontSize:12, color:HOT, fontFamily:"'DM Sans',sans-serif", marginTop:2, opacity:0.75 }}>{t.dates} · {t.members} ladies</div></div>
          <div style={{ fontSize:11, fontWeight:700, padding:"4px 10px", borderRadius:20, background:t.status==="Planning"?`rgba(46,125,50,0.1)`:SOFT, color:t.status==="Planning"?GREEN:HOT, border:`1px solid ${t.status==="Planning"?"rgba(46,125,50,0.25)":MID}`, fontFamily:"'DM Sans',sans-serif" }}>{t.status}</div>
        </div>
      ))}
      ...
      {[["🔔","Deal Alerts","Flights, Airbnb & Vrbo price drops",true,true],["📧","Email Updates","Weekly destination deals",true,false],["💳","Payment Methods","Add cards for fast checkout",false,false],["🔒","Privacy","Manage your data",false,false]].map(([icon,label,sub,hasToggle,isOn])=>(
        <div key={label} style={{ ...C, marginBottom:8, display:"flex", alignItems:"center", gap:14, cursor:"pointer" }}>
          ...
          <div style={{ flex:1 }}><div style={{ fontSize:13, fontWeight:600, fontFamily:"'DM Sans',sans-serif", color:DARK }}>{label}</div><div style={{ fontSize:11, color:"#bbb", fontFamily:"'DM Sans',sans-serif" }}>{sub}</div></div>
